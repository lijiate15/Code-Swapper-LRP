...
用法:
  双击运行（弹窗模式）
  或命令行：python apply_changes.py <源脚本路径> <指令文档路径>
"""
...
def select_files_via_dialog() -> tuple[Path, Path]:
    ...
    instruction_str = filedialog.askopenfilename(
        ...
        filetypes=[("Markdown 文档", "*.md"), ("文本文件", "*.txt"), ("所有文件", "*.*")],
    )
    root.destroy()
...
def parse_instruction_doc(content: str) -> list[dict]:
    ...
    for i in range(1, len(splits), 2):
        ...
        # 检查是否为 SKIP 块
        if re.search(r'\[SKIP[^\]]*\]', block, re.IGNORECASE):
            results.append({
                "index": idx,
                "skip": True,
                "locate": "",
        ...
        if not locate_match or not replace_match:
            raise ValueError(
                ...
                f"请检查该块格式是否正确（必须包含 ```代码块``` 包裹的代码）。"
            )

        results.append({
            "index": idx,
            "skip": False,
            "locate": locate_match.group(1),
...
def main():
    ...
    # ---------- 归档指令文档 ----------
    changelog_path = target_path.parent / (target_path.stem + "_changelog.md")
    try:
        shutil.copy2(instruction_path, changelog_path)
        print(f"  ✅ 已归档指令文档: {changelog_path.name}")
    except Exception as e:
        print(f"  ⚠️  指令文档归档失败（不影响主流程）: {e}")
