...
1_14 版本变更:
  - change-log 预处理正则同时识别中英两套块名（修改说明/说明结束 与 change-log/end-of-change-log）
  - 修复 Boundary 模式锚行前导缩进被吃掉的 bug（原 \\s* + .strip() 导致带缩进锚行永不匹配）
  - Boundary 模式新增中间锚行（Start line +1 / +2 / ...）的解析与连续性校验，
    确保首尾锚行不仅各自唯一，还必须属于同一段连续代码块（向后兼容：不写中间锚行时按旧逻辑）
"""
...
def parse_instruction_doc(content: str) -> list[dict]:
    ...
    for i in range(1, len(splits), 2):
        ...
        # 注意：用 [ \t]? 而不是 \s*，只吃掉冒号后约定的单个分隔空格/制表符；
        # 后面的 (.*) 原样保留，包括锚行的前导缩进——这是 Boundary 模式
        # 能在缩进的代码上正常工作的前提。
        ...
        # 解析中间锚行（可选）：Start line +1 / Start line +2 / ...
        # 数量可变；仅按编号顺序收集，不要求连续编号。
        middle_line_matches = re.findall(
            r'^Start line\s*\+\s*(\d+):[ \t]?(.*)$',
            block,
            re.IGNORECASE | re.MULTILINE
        )
        # 按编号升序排列，取每条的内容（保留原缩进）
        middle_lines = [
...
def precheck(source_content: str, modifications: list[dict]) -> tuple[bool, list[str]]:
    ...
    for mod in modifications:
        ...
        if mod.get("mode", "locate") == "boundary":
            # 定位行模式预检查：
            #   1. 首尾定位行各自必须在源文件中唯一匹配；
            #   2. 尾行必须在首行之后；
            #   3. 若声明了中间锚行（Start line +1 / +2 / ...），
            #      则首行 + 中间锚行必须在源文件中按声明顺序连续出现，
            #      且最后一条中间锚行的下一行不能晚于尾行。
            #      （这一步把"几行连续锚定"的安全意图真正落地。）
